...
export async function loadMoneyPrioritySnapshot(householdId: string): Promise<MoneyPrioritySnapshot> {
  ...
  const [people, income, expenses, accounts, debts, retirementAccounts, goals, insuranceExposures, preferences, hsaTaxYearProfiles, hsaMonthStatuses, hsaMarriedAllocations] = await Promise.all([
    ...
    supabase.from("retirement_accounts").select("id, owner_person_id, name, account_type, balance, monthly_employee_contribution, monthly_employer_contribution, tax_treatment, employee_contributed_ytd, employer_contributed_ytd, hsa_ytd_tax_year, annual_contribution_target, full_match_employee_contribution_monthly, match_status, hsa_coverage_type, hsa_eligible, simple_higher_limit_eligible, simple_plan_limit_category, simple_plan_limit_tax_year, employer_contribution_type, plan_eligible_compensation_annual, prior_year_sponsor_wages, roth_catch_up_supported, sep_eligible_compensation_annual, sep_compensation_calculation_supported").eq("household_id", householdId).order("created_at"),
    supabase.from("goals").select("id, name, target_amount, current_amount, target_date, priority, goal_class, necessity, deadline_flexibility, consequence_level, planned_monthly_contribution, core_need_amount, goal_intelligence_confirmed, underlying_need, desired_solution, goal_nature, underfunding_consequence, borrowing_likelihood, expected_borrowing_amount, expected_borrowing_apr").eq("household_id", householdId).order("created_at"),
