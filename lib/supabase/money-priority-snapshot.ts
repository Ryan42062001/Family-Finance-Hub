...
export async function loadMoneyPrioritySnapshot(householdId: string): Promise<MoneyPrioritySnapshot> {
  ...
  const [people, income, expenses, accounts, debts, retirementAccounts, goals, insuranceExposures, preferences] = await Promise.all([
    supabase.from("household_people").select("id, display_name, relationship, birth_date, planned_retirement_age, is_dependent, is_active").eq("household_id", householdId).order("created_at"),
    ...
    supabase.from("expenses").select("id, name, category, monthly_amount, is_essential").eq("household_id", householdId).order("created_at"),
