...
function snapshotToRaw(snapshot: MoneyPrioritySnapshot): MoneyPriorityRawSnapshot {
  return {
    ...
    goals: snapshot.goals.map((item) => ({
      ...
      goal_intelligence_confirmed: item.goalIntelligenceConfirmed,
      underlying_need: item.underlyingNeed,
      desired_solution: item.desiredSolution,
      goal_nature: item.goalNature,
      underfunding_consequence: item.underfundingConsequence,
      borrowing_likelihood: item.borrowingLikelihood,
      expected_borrowing_amount: item.expectedBorrowingAmount,
      expected_borrowing_apr: item.expectedBorrowingApr,
    })),
