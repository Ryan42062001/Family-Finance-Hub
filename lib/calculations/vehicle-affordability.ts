...
export type VehicleAffordabilityState =
  | "comfortably_affordable"
  | "affordable"
  | "stretch"
  | "not_recommended"
  | "more_information_needed";
export type VehicleFinancingQuality =
  | "preferred"
  | "acceptable"
  | "caution"
  | "not_recommended"
...
export type VehicleScenarioComparison<T extends VehiclePurchaseScenario = VehiclePurchaseScenario> = {
  scenario: T;
...
function assessFinancing(
...
): VehicleFinancingAssessment {
  if (amountFinanced <= 0) {
    return {
      quality: "not_applicable",
      aprSignal: "not_applicable",
      termSignal: "not_applicable",
      equitySignal: "not_applicable",
      reasons: ["No vehicle loan is required."],
    };
  }
  if (apr === null || termMonths === null || !finiteNonnegative(apr) || !Number.isInteger(termMonths) || termMonths <= 0) {
    return {
      ...
      aprSignal: "unknown",
      termSignal: "unknown",
      equitySignal: loanToValue !== null && loanToValue > 1 ? "underwater" : "ordinary",
      reasons: ["A valid APR and positive whole-month loan term are required to assess financing."],
    };
  }

  const decimalApr = apr / 100;
  const aprSignal = decimalApr >= policy.highInterestDebtApr
    ? "high_interest"
    : decimalApr >= policy.payoffFavoredDebtApr
      ? "payoff_favored"
      : decimalApr >= policy.grayZoneDebtApr
        ? "gray_zone"
        : "low";
  const termSignal = termMonths <= policy.vehicleAffordability.preferredTermMaxMonths
    ? "preferred"
    : termMonths <= policy.vehicleAffordability.normalTermMaxMonths
      ? "normal"
      : termMonths <= policy.vehicleAffordability.cautionTermMaxMonths
        ? "caution"
        : "strong_caution";
  const equitySignal = loanToValue === null
    ? "not_applicable"
    : loanToValue <= 0.8
      ? "strong"
      : loanToValue <= 1
  ...
  const reasons = [
    aprSignal === "high_interest"
      ? "The APR meets the existing Secure-stage high-interest debt threshold."
      : aprSignal === "payoff_favored"
        ? "The APR falls in the existing payoff-favored debt range."
        : aprSignal === "gray_zone"
          ? "The APR falls in the existing debt-versus-investing gray zone."
          : "The APR is below the ordinary debt-acceleration thresholds.",
    termSignal === "preferred"
      ? `The term is within ${policy.vehicleAffordability.preferredTermMaxMonths} months.`
      : termSignal === "normal"
        ? `The term is within ${policy.vehicleAffordability.normalTermMaxMonths} months.`
        : termSignal === "caution"
          ? `The term exceeds ${policy.vehicleAffordability.normalTermMaxMonths} months and warrants caution.`
          : `The term exceeds ${policy.vehicleAffordability.cautionTermMaxMonths} months and warrants strong caution.`,
  ];
...
export function evaluateVehicleAffordability(
  ...
  const requiredAmounts: Array<[string, number | null]> = [
    ...
    ["tradeInLoanPayoff", scenario.tradeInLoanPayoff],
    ["cashDownPayment", scenario.cashDownPayment],
    ["salesTax", scenario.salesTax],
    ["titleRegistrationFees", scenario.titleRegistrationFees],
    ["otherPurchaseFees", scenario.otherPurchaseFees],
    ["monthlyInsuranceChange", scenario.monthlyInsuranceChange],
    ["monthlyFuelChange", scenario.monthlyFuelChange],
    ["monthlyMaintenanceChange", scenario.monthlyMaintenanceChange],
    ["monthlyRegistrationTaxChange", scenario.monthlyRegistrationTaxChange],
    ["monthlyParkingTollsChange", scenario.monthlyParkingTollsChange],
  ];
  for (const [name, value] of requiredAmounts) {
    if (value === null || !Number.isFinite(value)) missingData.push(name);
  }
  for (const [name, value] of requiredAmounts.slice(0, 7)) {
    if (value !== null && Number.isFinite(value) && value < 0) missingData.push(`${name} must not be negative`);
  }

  const taxesAndFees = roundMoney(
    (scenario.salesTax ?? 0) + (scenario.titleRegistrationFees ?? 0) + (scenario.otherPurchaseFees ?? 0),
  );
  ...
  const monthlyPayment = amountFinanced > 0 && scenario.loanApr !== null && scenario.loanTermMonths !== null
    && finiteNonnegative(scenario.loanApr) && Number.isInteger(scenario.loanTermMonths) && scenario.loanTermMonths > 0
    ? calculatePayment(amountFinanced, scenario.loanApr, scenario.loanTermMonths)
    : 0;
  ...
  const loanToValue = scenario.purchasePrice > 0 && amountFinanced > 0
    ? Math.round((amountFinanced / scenario.purchasePrice) * 10000) / 10000
    : null;
  const acquisition: VehicleAcquisitionAssessment = {
    tradeEquity,
    negativeEquityRolledIn,
    netAcquisitionCost,
    amountFinanced,
    monthlyPayment,
    totalLoanPayments,
  ...
  const operatingCostChange = roundMoney(
    (scenario.monthlyInsuranceChange ?? 0)
      + (scenario.monthlyFuelChange ?? 0)
      + (scenario.monthlyMaintenanceChange ?? 0)
      + (scenario.monthlyRegistrationTaxChange ?? 0)
      + (scenario.monthlyParkingTollsChange ?? 0),
  );
  const allInMonthlyImpact = roundMoney(monthlyPayment + operatingCostChange);
  const postPurchaseCashFlow = roundMoney(engine.snapshot.aggregates.monthlyCashFlowBeforeSavings - allInMonthlyImpact);
  ...
  const monthlyImpact: VehicleMonthlyImpact = {
  ...
  const availableVehicleCash = roundMoney(
    Math.max(0, unallocatedAfterPriority - engine.existingCash.liquidityFloor) + relatedGoalEarmarkedCash,
  );
  ...
  const cash: VehicleCashAssessment = {
    unallocatedCashBeforeVehicle: engine.existingCash.availableUnallocatedCash,
    higherPriorityOneTimeDeployments,
    relatedGoalEarmarkedCash,
    requiredLiquidityFloor: engine.existingCash.liquidityFloor,
    availableVehicleCash,
    cashRequired,
  ...
  const secureNeedsPreserved = engine.feasibility.status !== "funding_gap" && postPurchaseCashFlow >= secureMonthlyNeed;
  ...
  const prePurchasePlanMargin = roundMoney(engine.snapshot.aggregates.monthlyCashFlowBeforeSavings - recurringClaims);
  const retirementMonthlyAllocationDisplaced = roundMoney(Math.min(
    retirementAllocation,
    Math.max(0, allInMonthlyImpact - Math.max(0, prePurchasePlanMargin)),
  ));
  const retirementMateriallyWorsened = retirementMonthlyAllocationDisplaced > 0;
  const planImpact: VehiclePlanImpact = {
  ...
  if (missingData.length) {
    ...
    if (negativeCashFlow || protectedCashFailure || hardPlanFailure || optionalNegativeEquity
      || (highApr && scenario.needType !== "necessary")) {
      ...
      else if (thinMargin || retirementMateriallyWorsened || negativeEquityRolledIn > 0
        || financing.quality === "caution") affordability = "stretch";
    ...
    if (!employerMatchPreserved) risks.push("The purchase prevents full employer-match capture.");
    if (!secureNeedsPreserved) risks.push("The purchase displaces required Secure-stage funding.");
    if (!requiredGoalsPreserved) risks.push("The purchase makes an unrelated required or protective goal infeasible.");
    ...
    if (retirementMateriallyWorsened) risks.push("The purchase displaces a modeled recurring retirement allocation.");
    if (financing.termSignal === "caution" || financing.termSignal === "strong_caution") {
      risks.push("The longer loan term lowers the payment but increases duration and total financing exposure.");
    }
    if (loanToValue !== null && loanToValue > 1) risks.push("The amount financed exceeds the vehicle value.");
    if (scenario.coreNeedAmount !== null && scenario.coreNeedAmount !== undefined
      && scenario.purchasePrice > scenario.coreNeedAmount) {
      risks.push("The selected vehicle price exceeds the explicitly supplied core transportation need.");
    }
    reasons.push(
      affordability === "not_recommended" && scenario.needType === "necessary"
        ? "Transportation appears necessary, but this purchase structure would compromise higher-priority financial needs."
        : "Affordability reflects the all-in monthly impact after preserving the existing financial-priority plan.",
    );
  }
...
const affordabilityRank: Record<VehicleAffordabilityState, number> = {
...
const financingRank: Record<VehicleFinancingQuality, number> = {
  preferred: 0,
  acceptable: 1,
  caution: 2,
  not_applicable: 3,
  more_information_needed: 4,
  not_recommended: 5,
};

export function compareVehiclePurchaseScenarios<T extends VehiclePurchaseScenario>(
  engine: MoneyPriorityEngineResult,
  scenarios: readonly T[],
  policy: MoneyPriorityPolicy = MONEY_PRIORITY_POLICY_V1,
): VehicleScenarioComparison<T>[] {
  return scenarios
    .map((scenario) => ({ scenario, result: evaluateVehicleAffordability(engine, scenario, policy) }))
    .sort((a, b) =>
      affordabilityRank[a.result.affordability] - affordabilityRank[b.result.affordability]
      || financingRank[a.result.financing.quality] - financingRank[b.result.financing.quality]
      || a.result.acquisition.totalInterest - b.result.acquisition.totalInterest
      || a.result.acquisition.netAcquisitionCost - b.result.acquisition.netAcquisitionCost
