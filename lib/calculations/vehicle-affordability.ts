...
export function evaluateVehicleAffordability(
  ...
  const monthlyPayment = amountFinanced > 0
    && scenario.loanApr !== null
    && scenario.loanTermMonths !== null
    && finiteNonnegative(scenario.loanApr)
    && Number.isInteger(scenario.loanTermMonths)
    && scenario.loanTermMonths > 0
  ...
  if (missingData.length === 0 && protectedCashRequired === 0) {
    postEngine = runHypotheticalMoneyPriorityEngine(engine, {
      ...
      addDebts: amountFinanced > 0 ? [{
        id: "hypothetical-vehicle-loan",
        name: "Hypothetical vehicle loan",
        type: "auto",
        balance: amountFinanced,
        annualInterestRate: scenario.loanApr,
        minimumPayment: monthlyPayment,
        rateType: "fixed",
      }] : [],
      addExpenses: operatingCostChange !== 0 ? [{
        id: "hypothetical-vehicle-operating-cost",
        name: "Hypothetical vehicle operating cost change",
        category: "transportation",
        monthlyAmount: operatingCostChange,
        isEssential: scenario.needType === "necessary",
        cashFlowTreatment: scenario.needType === "optional_upgrade" ? "discretionary" : "required",
      }] : [],
  ...
  const postPurchaseCashFlow = postEngine
    ...
    : roundMoney(engine.snapshot.aggregates.monthlyCashFlowBeforeSavings - allInMonthlyImpact);
  const postProtectedClaims = postEngine
    ? protectedRecurringAllocations(postEngine, relatedGoalId)
    : protectedRecurringAllocations(engine, relatedGoalId);
  const postPurchasePlanMargin = roundMoney(postPurchaseCashFlow - postProtectedClaims);
  ...
  if (missingData.length) {
    ...
    if (negativeCashFlow || protectedCashFailure || hardPlanFailure || optionalNegativeEquity || (highApr && scenario.needType !== "necessary")) {
    ...
    reasons.push(
      affordability === "not_recommended" && scenario.needType === "necessary"
        ? "Transportation appears necessary, but the authoritative post-purchase plan shows this purchase structure would compromise higher-priority financial needs."
