...
alter table public.household_financial_preferences
  ...
  add constraint household_financial_preferences_tax_profile_year_check
    check (tax_profile_year is null or tax_profile_year between 2020 and 2100),
  drop constraint if exists household_financial_preferences_tax_filing_status_check,
  add constraint household_financial_preferences_tax_filing_status_check
    check (tax_filing_status is null or tax_filing_status in ('single', 'head_of_household', 'married_filing_jointly', 'married_filing_separately')),
  drop constraint if exists household_financial_preferences_estimated_modified_agi_check,
  add constraint household_financial_preferences_estimated_modified_agi_check
    check (estimated_modified_agi is null or estimated_modified_agi >= 0);
...
alter table public.household_people
  ...
  add constraint household_people_estimated_taxable_compensation_annual_check
    check (estimated_taxable_compensation_annual is null or estimated_taxable_compensation_annual >= 0);
