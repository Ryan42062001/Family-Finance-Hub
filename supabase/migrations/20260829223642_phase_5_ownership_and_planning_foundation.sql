...
alter table public.income_sources
  ...
  add constraint income_sources_owner_person_household_fkey
...
alter table public.retirement_accounts
  ...
  add constraint retirement_accounts_owner_person_household_fkey
    foreign key (owner_person_id, household_id)
    references public.household_people(id, household_id)
    on delete set null (owner_person_id);

alter table public.retirement_accounts drop constraint retirement_accounts_account_type_check;
alter table public.retirement_accounts add constraint retirement_accounts_account_type_check
  check (account_type in ('401k','403b','457','traditional_ira','roth_ira','hsa','pension','tsp','simple_ira','sep_ira','other'));
